...
public class ValutaFrissitesThread extends Thread {
    ...
    public void run() {
        ...
        Query query = new Query.QueryBuilder()
                ...
                .where("valuta != 'HUF'" + (!mind ? " && DATEDIFF(NOW(), utolso_modositas) >= 1" : ""))
                .build();
        Object[][] select = App.db.select(query.getQuery());
        Object[] object = new Object[1];
        for (int i = 0; i < select.length; i++) {
            object[0] = arfolyam(String.valueOf(select[i][0]));
            App.db.insert("UPDATE szamlazo_valuta SET kozeparfolyam = ?, utolso_modositas = NOW() WHERE valuta = '" + String.valueOf(select[i][0]) + "'", object, 1);
        }
        kesz = true;
    }
